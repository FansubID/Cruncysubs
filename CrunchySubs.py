...
"lenguaje":"'''+unicode(lang, "ISO-8859-1")+'''"
    ...
    def getUrl(self, url):
    	...
    	if not subtitulos:
    		self.addError("Kesalahan: Anda harus menjadi pengguna premium untuk mengunduh subtitle ini.")
    		return False
    	subs_lang = []
    	subs_url = []
    	for i in subtitulos:
    ...
    def UrlCheck(self,url):
        ...
        if re.match(r"^((?:http|https):\/\/)(w{3}\.)?(crunchyroll.com\/[a-zA-Z0-9\-\_]+\/)((?:[a-zA-Z0-9\-\_]+)\-[0-9]+)",url):
            ...
            if not media_id:
            	self.addError("Kesalahan: Tautan ini pasti Crunchyroll/Video, tetapi tampaknya tidak benar atau episode tidak tersedia.")
            	return False
                    ...
                    while sett:
                        ...
                        if option == "1":
                            ...
                            while True:
                                settings = subs.LoadSettings()
                                ayuda = ''' 
None =Akan menampilkan daftar subtitle yang tersedia yang dapat Anda pilih.
* = Unduh semua bahasa yang tersedia.
Anda dapat memilih bahasa secara default, misalnya "Español". Ini harus bertepatan dengan bahasa yang ditampilkan di halaman Crunchroll.
Beberapa lagu yang saya tahu:
-) English (US)
-) Español
-) Español (España)
-) Italiano
